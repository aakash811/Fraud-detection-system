...
A production-grade system that detects fraud from payment transactions, applies detection rules, persists decisions in PostgreSQL, and streams everything live to an ops dashboard — all connected through Kafka.
...
Payment Source / Merchant
  ...
  Fraud Analyzer (port 8081)  ←── consumes transactions.v1
        ...
        ↓
  Risk Orchestrator (port 8083)
        | consumes decisions.v1
...
Alert Correlation — preventing duplicate alerts
...
The first few failures are counted.
Once the threshold is crossed, the circuit opens — further requests fast-fail immediately instead of waiting for timeouts.
After a configured interval, the circuit half-opens to probe recovery.
...
Kafka Integration
The Fraud Analyzer uses Kafka as the backbone for event streaming:

@KafkaListener(topics = "transactions.v1", groupId = "fraud-analyzer")
Consumes transaction events at high throughput
Publishes decisions.v1 with fraud scores and actions
Publishes fraud-alerts.v1 only for high-risk (score ≥ 0.9)
...
POST | /api/v1/investigations/blacklist/{cardLast4} | Add card to blacklist
...
This handles: start Kafka, Redis, PostgreSQL, Prometheus, Grafana.
...
Helm Chart — charts/fraud-detection/
